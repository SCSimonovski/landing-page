...
- **Per-product qualifying data lives in `leads.details` (JSONB), not as top-level columns.** `brand` and `product` are text discriminators with app-level Zod validation, no DB-level enum (flexibility to add brands without DDL). Top-level columns on `leads` stay reserved for brand-agnostic demographics (state, age, names, contact) + control fields (status, agent_id, on_dnc) + attribution (utm_*, fbclid). Per-product fields like `mortgage_balance`, `is_smoker`, `is_homeowner` (Northgate) or coverage amount + health questions (Final Expense) live inside `details`. Type safety on `details` reads comes from product-specific interfaces in `packages/shared/types/products.ts` plus runtime assertions at SMS/email template sites (see `formatAgentSMS` in `packages/shared/twilio/messages.ts` for the pattern). Plan 2 will split per-product templates so the runtime assertion is replaced by per-product Zod parsing in dispatchers routed by `lead.product`.
...
**Plan 2: scaffold `apps/final-expense/`.** Plan 1 (multi-brand schema migration) shipped on 2026-05-03 with byte-parity verification against Northgate's pre-migration behavior — see CHANGELOG. The schema now supports multiple consumer brands via `leads.brand` + `leads.product` discriminators + `leads.details` JSONB. Plan 2 adds the second consumer brand under the same LLC: a Final Expense lead-gen app that submits with `brand='final-expense'`, `product='final_expense'`, and FE-specific qualifying details. Architect-locked spec: directory `apps/final-expense/`, full FE qualifying set (5–7 fields including age, smoker, desired coverage, basic health questions, beneficiary relationship), FE-shaped intent score, FE-tailored SMS template, repurposed second-choice UI direction from prior Northgate brand pass. Plan 2 also splits `formatAgentSMS` from one mortgage-only function with a runtime assertion to per-product templates routed by `lead.product` (Northgate template stays in shared, FE template added).

Subsequent tasks (rough order, not committed): Meta Pixel client-side install (now blocked on second-brand-app being live so Pixel can verify against the right URL); server-side Meta CAPI dispatch (`/api/leads` `Promise.all` third entry); daily DNC scrub cron (populates `dnc_registry` from the FTC list); replace placeholder copy + draft consent text + draft `/privacy` + `/terms` content with attorney-reviewed final versions; register custom consumer domains for both brands (gates on LLC + brand); `mpl-prod` Supabase project + baseline migration replay (deferred until launch is imminent — free-tier projects pause after 7 days of inactivity).
