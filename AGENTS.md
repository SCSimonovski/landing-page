...
- **Role-aware RLS pattern (post-Plan-3).** Platform reads use role-aware policies keyed off `platform_users.role` (`agent` / `admin` / `superadmin`). `agents` is agents-only (admin/superadmin live in `platform_users` only — no paired `agents` row). All policies call the `current_platform_user()` SECURITY DEFINER function to look up role + agent_id without recursing into themselves (naive nested `exists` against same table → Postgres 42P17 infinite recursion). The function has `set search_path = public` — load-bearing protection against search-path injection on SECURITY DEFINER. `auth.jwt()` calls inside the function are wrapped in `(select ...)` for InitPlan execution.
- **RPC-only-writes pattern (post-Plan-5).** Row-scoped writes from `authenticated` (status updates, lead assignment, profile changes, active toggle) flow through SECURITY DEFINER RPCs (`update_lead_status`, `assign_lead`, `update_lead_notes`, `set_platform_user_active`, `update_agent_profile`) — NOT through direct UPDATE grants. Why: RLS handles row-scoping but not column-scoping; with a generic UPDATE grant, an authenticated agent could craft `.from('leads').update({ on_dnc: true })` from the browser console. RPCs bundle authorization + atomic update + audit (`lead_events` insert with `actor_platform_user_id`) in one transaction. New row-scoped write surfaces should follow the same pattern: a new RPC with `set search_path = public`, EXECUTE granted to `authenticated`, no direct UPDATE grants. Anti-grant verification: `scripts/test-platform-rls.ts` includes an assertion that `agent.from('leads').update(...)` returns permission denied — guards against future migrations accidentally adding the grant.
