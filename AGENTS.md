...
**Brand identity:** Northgate Protection brand (deep navy `#102841`, cream `#F2EBDC`, gold-brown accent `#7A6A4F`, Georgia serif headings + system sans body) applied across landing page, header, footer, form. Logo at `public/northgate-logo.svg`. All placeholder copy on `/` finalized per brand brief — `[HEADLINE PLACEHOLDER]` and FAQ `[Placeholder answer]` slots are gone; testimonials section deleted entirely (no fake testimonials before real customer outcomes). Hero image slot is a dev-only placeholder treatment (cream gradient + arch watermark) — real photo required before SAC submission.
...
- **Real hero photo before SAC submission.** Per playbook 02 Part 2.5, real photos beat stock by ~20% and shipping with no photo at all underperforms even stock. Current state: hero image slot reserves the layout space with a cream-gradient + arch-watermark placeholder; intentional for dev/preview, not acceptable for the Meta ad SAC review. Source an architectural-detail shot (porch column, doorway, mailbox, window detail — easy to find on Unsplash without looking stock) and drop it into `public/hero.jpg`; one Edit on `src/app/page.tsx` to wire the `<img>` tag.
