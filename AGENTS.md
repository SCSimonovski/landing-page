...
- **Hero imagery for Heritage: firm SAC blocker, NOT a watch-item.** Hearth visual direction *expects* photography. Plan 2b ships a placeholder asset at `apps/northgate-heritage/public/hero-placeholder.svg` plus a dev-only banner gated on `NEXT_PUBLIC_HERO_PLACEHOLDER` env var (set to `true` in local dev / preview; UNSET in production env so the banner doesn't render). **Real photography per Hearth direction is a hard prerequisite for any paid Heritage SAC** — not a launch-readiness gate (Heritage launch is gated on much more), but Heritage cannot run paid ads with the placeholder still rendering. Once real photography lands, swap `<HeritageHero>` to render the real image and remove the `NEXT_PUBLIC_HERO_PLACEHOLDER` env var (becomes a 5-place removal task per the operational discipline rule).
