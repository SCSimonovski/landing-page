...
**Migration `20260511120000_add_platform_users_full_name_and_backfill.sql`.** Adds `platform_users.full_name text` (nullable for now), backfills from `agents.full_name` via a `from agents` UPDATE for every existing role=agent row, manually backfills the one existing superadmin. The migration intentionally does NOT modify `current_platform_user()` — Postgres rejects `CREATE OR REPLACE FUNCTION` when the return-table shape changes (SQLSTATE 42P13), and `DROP FUNCTION ... CASCADE` would torch ~10+ RLS policies that depend on it (across `platform_users`, `agents`, `leads`, `consent_log`, `lead_events`). Application-side fix is much smaller: `getPlatformUser()` does a second SELECT against `platform_users` by id to pull `full_name` after the existing RPC call. Same JWT context; the user's own platform_users SELECT policy from Plan 3 covers the read.

**Source of truth.** `platform_users.full_name` is canonical going forward. Agent invite path writes the value to `platform_users.full_name`; the dual-write to `agents.full_name` stays only because the column is still NOT NULL — a follow-up migration drops the column entirely. Reads switched to `platform_users.full_name` at every site that needs to render a non-agent's display name (sidebar footer via `getPlatformUser().fullName` — second SELECT inside the helper; activity timeline actor via the changed nested-select `actor:platform_users!actor_platform_user_id(email, full_name)`; /users Full name column via `select("...full_name, agents(license_states)")`). Sites that read an assigned agent's full_name (assignment dropdowns, filter-bar agent options, bulk-action-bar, CSV export) keep reading `agents.full_name` — they're scoped to role=agent rows by definition, and the values stay in sync until the follow-up drop migration refactors them through `agents.platform_user_id → platform_users.full_name`.

**Named display-name helpers.** New `apps/northgate-leads/src/lib/auth/display-name.ts` exports `displayName(user)` (personal contexts: sidebar, "Assigned: …" pill, activity timeline → falls back to email so the user always sees something) and `displayNameOrDash(user)` (tabular contexts: /users Full name column → falls back to em-dash so an empty name reads as a clear gap, not duplicated email). Centralized so the convention lives in one place; if the fallback rule changes (e.g., a "click to add name" affordance for missing values) it's a single-site edit.

**Invite form / Zod.** `inviteSchema.full_name` is now `z.string().trim().min(1)` (required for every role; no `.optional()`). The agent-only `superRefine` check on `full_name` was removed (base validation handles it); only the `license_states` agent-only check remains. `<InviteUserDialog>` shows the Full name field unconditionally; License states stays agent-only.

**Verification.** `scripts/test-platform-rls.ts` extended with three new assertions (5b) — every role can SELECT its own `platform_users.full_name` via the existing platform_users SELECT policy (the read path `getPlatformUser()`'s second query takes). Guards against a future policy narrowing that would break the sidebar / actor display. Fixture `platform_users` insert now sets `full_name` on all five rows. `apps/northgate-leads/src/app/users/invite-schema.test.ts` updated for the new "required for every role" rule (admin without full_name now fails; admin/superadmin with full_name passes; agent rules unchanged).

**OOB follow-up.** Drop `agents.full_name` in a follow-up migration once this ships and the read-path refactor for assignment dropdowns / filter-bar / bulk-action-bar / CSV export lands. The drop migration must (a) refactor those reads to `agents.platform_user_id → platform_users.full_name`, (b) `alter table agents drop column full_name`, (c) re-run `pnpm gen:types`. Decoupled from this PR to keep the blast radius small if a rollback is needed.
